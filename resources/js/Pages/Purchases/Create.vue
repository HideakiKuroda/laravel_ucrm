...
const setCustomerId = (id,name,tel) => {
    form.customer_id = id
    form.customer_name = name
    form.customer_tel = tel
}
                      ...
                      <form @submit.prevent="storePurchase">
                        ...
                        <div class="container px-5 py-8 mx-auto">
                            ...
                            <div class="flex flex-wrap -m-2">
                                ...
                                <div class="p-2 lg:w-2/3">
                                <div class="relative">
                                    <label for="customer" class="leading-7 text-sm text-gray-600">会員名</label><br>
                                    <label class="w-full  bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                                        {{ form.customer_id }}
                                    </label>
                                    <label class="ml-2 w-full  bg-opacity-50  border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                                        {{ form.customer_name }}
                                    </label>
                                    <label class="ml-2 w-full  bg-opacity-50 border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                                        {{ form.customer_tel }}
                                ...
                                <div class="w-full mt-8 mx-auto overflow-auto">
                                    <table class="table-auto w-full text-left whitespace-no-wrap">
                                        ...
                                        <tr v-for="item in itemList" :key="item.id">
                                            ...
                                            <td class="border-b-2 border-gray-200 px-4 py-3 text-right">{{ item.price.toLocaleString() }}</td>
                                            <td class="border-b-2 border-gray-200 px-4 py-3 text-right">
                                            <select name="quantity" v-model="item.quantity">
                                            <option v-for="q in quantity" :value="q">{{ q }}</option>
                                            </select>
                                            </td>
                                            <td class="border-b-2 border-gray-200 px-4 py-3 text-right">{{ (item.price * item.quantity).toLocaleString() }}</td>
                                            </tr>
                                        </tbody>
                                    </table>
                                    <label for="customer" class="leading-7 text-sm text-gray-600">合計金額</label>
                                    <div class="text-right w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
